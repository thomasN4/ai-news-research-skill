...
**Regenerating.** When the digest is more than ~2 weeks stale, or research turns up major events past `coverage_end`, offer to update it. Check `updated_at` in the manifest first — Grok may have pushed recently, in which case pull before writing. Preserve the instructional comment block, update the `coverage-end` meta tag and the visible patch label, add new months, and **fix existing cards in place** when they turn out wrong or stale, saying what changed in the commit message. Keep the CONFIRMED/REPORTED tagging honest; a regenerated digest full of unverified claims is worse than a stale one. Upgrade REPORTED items to CONFIRMED when corroboration has appeared, and remove anything resolved from the gaps box, which lists only what is still open. Then bump `revision`, set `updated_by` to `claude`, and update `coverage_end` and `updated_at` in the manifest.
